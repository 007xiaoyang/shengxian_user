...
public class WebSocketUtil {
	...
	public void onMessage(String message ) throws IOException {
		...
		String token = jsonTo.getString("token");
		log.info("订单的店铺token："+token);
		if (!token.equals("All")){
			log.info("单方推送");
			sendMessageTo(mes, token );
		}else{
			sendMessageAll("给所有人");
			log.info("给所有人");
		}
	...
	public void sendMessageTo(String message, String token) throws IOException {
		// session.getBasicRemote().sendText(message);
		//session.getAsyncRemote().sendText(message);
		for (WebSocketUtil item : clients.values()) {
			if (item.token.equals(token) ){
				log.info("webSocket的店铺token："+item.token);
				log.info("订单的店铺token："+token);
				log.info("发送成功");
